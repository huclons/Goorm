...
module.exports = {
	...
	login: function(req, callback){
		if(req.body.auth_type=='password'){
			...
			if(!req.user){	//소셜로그인이 되어있다면 req.user 객체가 존재한다.
	...
	,get_login_status: function(req, callback){
		if(req.session.user){
			...
			//로그인 안되어 있을 때.
			//소셜로그인이 이루어져 있을 경우에는 해당 로그인 정보를 포함시킴
			...
			switch(auth_type){
				...
				case 'facebook':
					callback({
						...
						// , nick		: req.user.facebook.screen_name
						// , picture	: req.user.facebook.profile_image_url
					});
					break;
				case 'twitter':
					callback({
