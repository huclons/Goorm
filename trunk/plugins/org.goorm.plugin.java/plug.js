		...
		var bind_action_to_template_menu = function() {
			...
			$("a[action=new_java_package_action]").mousedown(function() {
				...
				if(filepath.indexOf("/src")>=0){
					...
					$("#java_package_name").val(filepath.substring(filepath.indexOf(source_path)+source_path.length+1).replace(/\//g,"."));
					$("#java_package_source_folder").val(source_path)
			...
			$("a[action=new_java_class_action]").mousedown(function() {
				...
				if(filepath.indexOf("/src")>=0){
					...
					var project_name = core.status.current_project_path;
					var source_path=project_name+"/src"
					$("#java_class_package").val(filepath.substring(filepath.indexOf(source_path)+source_path.length+1).replace(/\//g,"."))
					$("#java_class_source_folder").val(project_name+"/src")
			...
			$("a[action=new_java_interface_action]").mousedown(function() {
				...
				if(filepath.indexOf("/src")>=0){
					...
					$("#java_interface_package").val(filepath.substring(filepath.indexOf(source_path)+source_path.length+1).replace(/\//g,"."))
					$("#java_interface_source_folder").val(project_name+"/src")
...
//			self.set_breakpoints();
	...
	build: function (projectName, callback, is_auto_build, fileName, target) {
		...
		buildPath_guarantee_cmd+= 'if [ ! -d '+workspace+projectName+'/'+plugin['plugin.java.build_path']+' ];';
		buildPath_guarantee_cmd+= 'then mkdir -p '+workspace+projectName+'/'+plugin['plugin.java.build_path']+';';
	...
	init_dialog: function(){
		...
		var buttons_package = [ 
			{ id:"gPluginJavaPackageB_OK", text:"<span localization_key='ok'>OK</span>",  handler:handle_ok_package},
			{ id:"gPluginJavaPackageB_CNCL", text:"<span localization_key='cancel'>Cancel</span>",  handler:handle_cancel}
		];

		var buttons_interface = [
			{ id:"gPluginJavaInterfaceB_OK", text:"<span localization_key='ok'>OK</span>",  handler:handle_ok_interface},
			{ id:"gPluginJavaInterfaceB_CNCL", text:"<span localization_key='cancel'>Cancel</span>",  handler:handle_cancel}
		];

		var buttons_class = [
			{ id:"gPluginJavaClassB_OK", text:"<span localization_key='ok'>OK</span>",  handler:handle_ok_class},
			{ id:"gPluginJavaClassB_CNCL", text:"<span localization_key='cancel'>Cancel</span>",  handler:handle_cancel}
		];
		...
		dialog_class.init({
			...
			title:"Class", 
			path:"configs/dialogs/org.goorm.core.plugin/plugin.java.class.html",
			width:400,
			height:360,
			modal:true,
		...
		dialog_package.init({
			...
			title:"Package", 
			path:"configs/dialogs/org.goorm.core.plugin/plugin.java.package.html",
			width:400,
			height:200,
			modal:true,
			yes_text: "<span localization_key='ok'>OK</span>",
		...
		dialog_interface.init({
			...
			title:"Interface", 
			path:"configs/dialogs/org.goorm.core.plugin/plugin.java.interface.html",
			width:400,
			height:310,
			modal:true,
			yes_text: "<span localization_key='ok'>OK</span>",
