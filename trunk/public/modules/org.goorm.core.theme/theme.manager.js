	...
	create_datatable: function () {
		...
		$.each(self.treeview_json, function (index, object) {
			$.each(object.tab, function (index, object) {
				self.table_variable_array.push(new YAHOO.widget.ScrollingDataTable(object.element, table_variable_column_definition, table_variable_data_properties, {
					width: "694px",
					height: "310px"
				}));
		...
		for (var i = 0; i < element_array.length; i++) {
			...
			if ($.isArray(value)) {
				for (var style_cnt = 0; style_cnt < value.length; style_cnt++) {
					self.table_variable_array[table_idx].addRow({
						selector: selector,
						position: position,
						description: description,
						property: property,
						value: value[style_cnt],
						isArray: true
					}, 0);
				}
			} else
