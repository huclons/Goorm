...
org.goorm.core.preference.filetype = {
	...
	init: function () {
		...
		$(core).on('goorm_login_complete', function () {
			if (core.user && (core.user.level == 'Admin' || core.user.level == 'Owner' || core.user.level >= 2)) {
				self.readonly = false;
				// Buttons on dialog
				self.add_button = new YAHOO.widget.Button("filetype_add", {
					onclick: {
						fn: function () {
							self.add();
						}
					},
					label: '<span localization_key="add">Add</span>'
				});
				...
				self.save_button = new YAHOO.widget.Button("filetype_save", {
					onclick: {
						fn: function () {
							self.save();
						}
					},
					label: '<span localization_key="save">Save</span>'
				});
			} else {
				self.readonly = true;
				$('#filetype_add').hide();
		...
		if (filetype_detail.find(".file_extension").length !== 0) {
			...
			for (var i = 0; i < core.filetypes.length; i++) {
				if (core.filetypes[i].file_extension == old_extension) {
					...
					core.filetypes[i].editor = filetype_detail.find(".editor").attr("value");
					core.filetypes[i].type = filetype_detail.find(".type").attr("value");
					core.filetypes[i].mode = filetype_detail.find(".mode").attr("value");
			...
			if (found === false && new_extension !== "") {
				var temp = {
					...
					editor: filetype_detail.find(".editor").attr("value"),
					description: filetype_detail.find(".description").val(),
					type: filetype_detail.find(".type").attr("value"),
					mode: filetype_detail.find(".mode").attr("value")
				};
	...
	create_filetype_detail: function (extension_name, editor, description, type, mode) {
		...
		// "selected" decision.
		filetype_detail.find("option").each(function () {
