...
org.goorm.core.printer.design.prototype = {
	init: function (target, width, height, scale, parent) {
		...
		canvas.append("<div class='shapes' style='position:absolute; width:" + width + "px; height:" + height + "px; border:1px solid #ccc;'></div>"); //This is a grid layer which has grid background image and opacity
			...
			$(this.parent.objects).each(function (i) {
				...
				if ($(this)[0].type == 'line') {
					...
					if ($(this)[0].properties.sx) {
						sx = parseInt($(this)[0].properties.sx, 10);
					}

					if ($(this)[0].properties.sy) {
						sy = parseInt($(this)[0].properties.sy, 10);
					}

					if ($(this)[0].properties.ex) {
						ex = parseInt($(this)[0].properties.ex, 10);
					}

					if ($(this)[0].properties.ey) {
						ey = parseInt($(this)[0].properties.ey, 10);
					}
